...
function Terminal() {
    ...
    const helpCommand = () => {
        ...
        finalHelpDivText.innerHTML = 'Type one of the commands listed above. For eg.';
    ...
    const handleEnter = (event) => {
        if (event.keyCode == 13 || event.which == 13) {
            if (document.getElementById('input-command') != "undefined") {
                document.getElementById('span-class').setAttribute("contenteditable", "false");
        ...
        <div className='terminal-base-class' onClick={focusInput}>
            <div className='terminal-header-class'>
                ...
                <div className='terminal-title'>
                    <img src={File} style={{ width: '20px', height: '20px', verticalAlign: 'middle', marginRight: '8px' }} />
                    <div style={{ display: 'flex', alignItems: 'center', fontSize: '15px' }}>dhruvil — - zsh</div>
                </div>
            </div>
            <div className='terminal-shell-class' id="terminal-shell-class">
                <div>Welcome</div>
                <div>Starting the server ...</div>
                <div style={{ color: '#66C2CD' }}># Type 'help' to list all commands</div>
                <div className='input-command' id='input-command'>
                    <div className='prefix-command-line' style={{ marginRight: "5px" }}>dhruvil@Dhruvils-Mac ~ % </div>
                    <span onChange={(e) => setInput(e.target.value)} onKeyUp={handleEnter} contenteditable="true" className='span-class' id='span-class'></span>
                    <div onClick={focusInput} id='cursor-command-line' className='cursor-command-line'></div>
                </div>
                <div>
